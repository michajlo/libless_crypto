...
int aes_init(aes_t *aes, uint32_t aes_type, uint8_t *key, uint8_t key_len) {
    ...
    if (aes_type == AES_128) {
        aes->key_size = 16;
        aes->expanded_key_len = 176;
        aes->num_rounds = 10;
        aes->expanded_key = (uint8_t *)malloc(aes->expanded_key_len);
    } else {
        return 1;
    }
    ...
    pad = aes->key_size - key_len;

    memcpy(aes->expanded_key, key, key_len);
    if (pad > 0) {
        memset(aes->expanded_key + key_len, '\0', pad);
    }

    expanded_len = aes->key_size;

    while (expanded_len < aes->expanded_key_len) {
        uint32_t t;
        uint32_t i;
        uint8_t *t_bytes;
        ...
        t_bytes[0] ^= rcon[I];
        ++I;
        t ^= *((uint32_t *)(aes->expanded_key + (expanded_len - aes->key_size)));
        memcpy(aes->expanded_key + expanded_len, &t, 4);
        expanded_len += 4;
        ...
        if (aes->key_size != AES_128) {
            uint32_t times = (aes->key_size == AES_192) ? 2 : 3;
            for (i=0; i<times; i++) {
                t ^= *((uint32_t *)(aes->expanded_key + (expanded_len - aes->key_size)));
                memcpy(aes->expanded_key + expanded_len, &t, 4);
                expanded_len += 4;
...
void add_round_key(aes_t *aes, uint32_t round) {
    ...
    for (i=0; i<4; i++) {
        state_32[i] = state_32[i] ^ key_32[i];
    }
}
...
void mix_columns(aes_t *aes) {
    ...
    for (i=0; i<16; i+=4) {
        ...
        result[0] = galois_2[aes->state[c0]] ^
                     ...
                     aes->state[c2] ^
                     aes->state[c3];
        result[1] = aes->state[c0] ^
                     galois_2[aes->state[c1]] ^
                     galois_3[aes->state[c2]] ^
                     aes->state[c3];
        result[2] = aes->state[c0] ^
                     aes->state[c1] ^
                     galois_2[aes->state[c2]] ^
                     galois_3[aes->state[c3]];
        result[3] = galois_3[aes->state[c0]] ^
                     aes->state[c1] ^
                     aes->state[c2] ^
                     galois_2[aes->state[c3]];
        aes->state[c0] = result[0];
        aes->state[c1] = result[1];
        aes->state[c2] = result[2];
        aes->state[c3] = result[3];
    }
}
...
void mix_columns_inv(aes_t *aes) {
    ...
    for (i=0; i<16; i+=4) {
        ...
        result[0] = galois_14[aes->state[c0]] ^
                     galois_11[aes->state[c1]] ^
                     galois_13[aes->state[c2]] ^
                     galois_9[aes->state[c3]];
        result[1] = galois_9[aes->state[c0]] ^
                     galois_14[aes->state[c1]] ^
                     galois_11[aes->state[c2]] ^
                     galois_13[aes->state[c3]];
        result[2] = galois_13[aes->state[c0]] ^
                     galois_9[aes->state[c1]] ^
                     galois_14[aes->state[c2]] ^
                     galois_11[aes->state[c3]];
        result[3] = galois_11[aes->state[c0]] ^
                     galois_13[aes->state[c1]] ^
                     galois_9[aes->state[c2]] ^
                     galois_14[aes->state[c3]];
        aes->state[c0] = result[0];
        aes->state[c1] = result[1];
        aes->state[c2] = result[2];
        aes->state[c3] = result[3];
